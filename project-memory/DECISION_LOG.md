...
| D005 | 2026-05 | FreshnessIndicator thresholds: ≤3d live, ≤14d recent, >14d stale | Aligns with typical vendor pricing update cadence; 3 days is within normal pipeline run frequency | 7d/30d thresholds | All pricing pages now show freshness state | Active |
...
| D007 | 2026-05 | Remove onClose from SearchBox mousedown listener | Calling onClose in a native DOM mousedown event handler flushes state before click fires on sibling elements, unmounting the menu | Keep onClose in mousedown, fix with stopPropagation | Mobile nav now works correctly | Active |
...
| D010 | 2026-05 | Remove min-w-[600px] from homepage savings grid | min-w-[600px] forced grid wider than 390px viewport — body overflow on mobile. grid-cols-2 is sufficient for mobile layout without a min-width constraint | Keep min-w and fix with max-w on body | Homepage mobile overflow eliminated; desktop unchanged | Active |
