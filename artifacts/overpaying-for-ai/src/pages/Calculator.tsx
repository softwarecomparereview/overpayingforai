...
export function Calculator() {
  ...
  const verdictRationale = verdict === "API"
    ? cheapestApi && cheapestSub
      ? `At ${formatTokenCount(inputTokens)} in / ${formatTokenCount(outputTokens)} out tokens, ${cheapestApi.model.name} (${formatCost(cheapestApi.monthlyCost)}/mo) undercuts ${cheapestSub.model.name} (${formatCost(cheapestSub.monthlyCost)}/mo).`
      : `Your usage profile favors per-token billing over a flat plan.`
    : cheapestApi && cheapestSub
      ? `At ${formatTokenCount(inputTokens)} in / ${formatTokenCount(outputTokens)} out tokens, ${cheapestSub.model.name} (${formatCost(cheapestSub.monthlyCost)}/mo) beats ${cheapestApi.model.name} (${formatCost(cheapestApi.monthlyCost)}/mo).`
  ...
  const escalationLine = !result
    ...
    : result.savingsEstimate && result.savingsEstimate > 0
      ? `Likely overpaying now: switching saves about ${formatCost(result.savingsEstimate)}/month.`
  ...
  const primaryRecommendation = verdictWinner
    ? `Recommended: ${verdictWinner.model.name} (${verdictWinner.model.provider}) — ${formatCost(verdictWinner.monthlyCost)}/month at your usage. ${escalationLine}`.trim()
